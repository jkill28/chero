...
export const Calendar: React.FC<CalendarProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pt-6 pb-4 bg-m3-surface-container border-b border-m3-outline/10">
        <div className="text-m3-on-surface-variant text-sm font-medium mb-1 uppercase tracking-wider">
          {t(language, 'selectDate') || 'Select date'}
        </div>
        <div className="text-m3-on-surface text-3xl font-semibold mb-6 flex justify-between items-end">
          <span>{format(selectedDate, 'EEE, MMM d', { locale })}</span>
        </div>

        <div className="flex items-center justify-between">
          <div className="text-m3-on-surface text-base font-semibold capitalize">
            {format(currentDate, 'MMMM yyyy', { locale })}
          </div>
          <div className="flex space-x-1">
            <button
              onClick={prevMonth}
              className="p-2 hover:bg-m3-on-surface-variant/10 text-m3-on-surface-variant rounded-full transition-colors"
            >
              <ChevronLeft size={24} />
            </button>
            <button
      ...
      <div className="grid grid-cols-7 px-2">
        {weekDays.map((day) => (
          <div
            key={day}
            className="h-12 flex items-center justify-center text-sm font-medium text-m3-on-surface-variant"
          >
            {day[0]}
          </div>
        ))}
        ...
        {calendarDays.map((day) => {
          ...
          return (
            <div
              ...
              className={cn(
                "min-h-[100px] sm:min-h-[160px] flex flex-col items-center pt-2 pb-1 relative cursor-pointer rounded-xl transition-colors group",
            ...
            >
              ...
              <div className={cn(
                ...
                isSelected
                  ? "bg-m3-primary text-m3-on-primary shadow-md scale-110"
                  : isToday
                    ? "border-2 border-m3-primary text-m3-primary"
              ...
              <div className="mt-1.5 flex flex-col items-center gap-0.5 px-1 w-full overflow-hidden">
                {balance !== undefined && (
                  <div className={cn(
                    "text-[10px] sm:text-[11px] font-black truncate",
              ...
              <div className="hidden sm:flex flex-col gap-1 w-full mt-2 px-1 pb-1 overflow-hidden">
                {transactions.map((occ, idx) => (
                  <div
                    ...
                    className={cn(
                      "text-[9px] px-1.5 py-0.5 rounded-m3-xs truncate transition-all hover:brightness-95 flex justify-between items-center",
              ...
              <div className="flex sm:hidden flex-wrap justify-center gap-0.5 mt-1 max-w-full px-0.5">
